        ...
        else if (type === 'sdp' || type === 'candidate' || type === 'offer' || type === 'answer')
        {
              ...
              for (const [userId, wss] of callers) {
               ...
               else if ( type === 'cancellcall')
               {
                ...
                console.log(" group chat accepted");
                    ...
                    const caller1 = clients.get(data.caller1); // user
                    const caller2 = clients.get(data.caller2);
                    const member = clients.get(data.from)
                    groupchat_members.delete(member)
           ...
           console.log("sending GroupChat member msg to ",data.to)
            ...
            console.log("Chat message received:", msg);
                            groupchat_members.forEach((ws, userId) => {
                                if (ws.readyState === ws.OPEN) {
                                    ws.send(JSON.stringify({ type: 'msg', msg: msg, chatsender: from }));
                                    console.log("Sending group chat message to", userId);
                                }
                            });
           ...
           member.send(JSON.stringify({ type: 'groupChat_ended' }));
            ...
            //ending groupchat for all user
            groupchat_members.forEach((ws, userId) => {
                if (ws.readyState === ws.OPEN) {
                    ws.send(JSON.stringify({ type: 'groupChat_ended' }));
                    groupchat_members.delete(member)
                    console.log("Sending end group chat message to", userId);
                }
            });
